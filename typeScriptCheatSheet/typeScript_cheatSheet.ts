...
var rad2deg: (rad: number) => number = function (radians) {
    return radians * (180 / Math.PI);
};//the () and => indicate the type is a function. (arguments)=>return type.

var nop: () => void = function () {
}; //or with the shorter lambda syntax  var nop:()=>void = ()=>{};
...
class ClassName extends BaseClassName implements InterfaceName {
    ...
    doSomething(f: string, f2: number) {
        return 0;
    }
...
var anonymousInterfaceImplementingVar: InterfaceName = {
    property1: "Test",
...
class Cat extends Zoo.Animal implements Zoo.IEatMeat {
    eat() {
...
var myAdd: (baseValue: number, increment: number) => number = function (x, y) {
    return x + y;
};
...
function defaultObjectParameterExample(firstname: string, userDetail = {
    op: "ADD",
    values: Array<number>(2, 3, 4, 5, 6)
}) { // -OR- values:[2,4,5,6,7]
...
var addCartItemFunc: (...items: any[]) => void = (...items: any[]) => {
    items.forEach((_) => {
        cart.items.push(_)
    })
};
...
class WordBank implements IList<string> {
    ...
    elementAt(n) {
        return this._items.length > n ? this._items[n] : null;
    }
...
concatFunc = <T>(arg1: T[], arg2: T[]) => {
    return arg2.concat(arg1);
}; //redefine it, same shape, reversed
console.log(concatFunc([1, 2, 3], [4, 5, 6]));//[4,5,6,1,2,3]
...
class DefaultComparer implements IComparer {
    ...
    Compare(element: any, otherElement: any): boolean {
...
class Tuple<T, U, V> {
    constructor(public arg1: T, public arg2: U, public arg3: V) {
...
var myTruthyStringImpl: IsTruthy<string> = (arg: string) => {
    return /^true$/i.test(arg) || arg == "1";
};
console.log(myTruthyStringImpl("TRUE"));//true
console.log(myTruthyStringImpl(""));//false
console.log(myTruthyStringImpl("1"));//true
var myTruthyNumberImpl: IsTruthy<number> = (arg: number) => {
    return !!arg;
};
...
class Nullable<T> {
    ...
    static IsNullable<U>(arg: U): boolean {
        return arg instanceof Nullable;
    } //but they can still use generics
    HasValue(): boolean {
        return this.Value != null;
    } //a method instead of a property because we cannot use getters and setters with default compilation settings.
    Value: T;
...
class DBConnection implements IDisposable { // Note: "implements IDisposable" is not required for our example. The type is checked without being explicit about it conforming to the interface.
    ...
    executeScalar(sql: string): string {
        return "Bob";
    };
}
...
class Fart {
    ...
    static createOne(str: string) {
        return new Fart();
    };
}
class WetFart extends Fart {
    play() {
        console.log("FFLLLPPPPPPPP....UH-OH...");
    };
